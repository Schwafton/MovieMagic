...
f = open(r"C:\Users\Schwa\Downloads\Flask_CRUD_App_Template_code\Flask_CRUD_App_Template_code\Flask_CRUD_App\static\movies.json", encoding="utf8")
...
conn = sqlite3.connect(r'C:\Users\Schwa\Downloads\Flask_CRUD_App_Template_code\Flask_CRUD_App_Template_code\CRUDdb.sqlite', check_same_thread=False)
...
app.config['MYSQL_DATABASE_DB'] = 'bloop'
'''
...
def movie():
    if 'loggedin' in session:
        ...
        cursor = conn.cursor()
        cursor.execute('SELECT Title, Genres FROM watched WHERE username = ?', (session['username'],))
        last_watched = cursor.fetchall()[-1:]
        recs = []
        last_watched_title = ""
        if len(last_watched) > 0:
            last_watched_title = last_watched[0][0]
            last_watched_genres = last_watched[0][1].split(',')

            for movie in Movies:
                if movie['title'] != last_watched_title:
                    if all(genre in movie['genres'] for genre2 in last_watched_genres):
                        recs.append(movie)
                        if len(recs) > 2:
                            break
        if query:
        ...
        return render_template('movies.html', movies=list_per_page, pagination=pagination, last=last_watched_title, recs=recs)
...
def history():
    ...
    if 'loggedin' in session:
        ...
        cursor.execute('SELECT DISTINCT keywords FROM searched WHERE username = ?', (session['username'],))
        recent_searches = cursor.fetchall()[-5:]
        # fetch genres
        cursor.execute('SELECT keywords, COUNT(DISTINCT title) FROM searched WHERE username = ? GROUP BY keywords', (session['username'],))
        all_searches = cursor.fetchall()
        genre_counts = {}
        for search in all_searches:
            for genre in Genres:
                if search[0].casefold() == genre.casefold():
                    genre_counts[genre] = search[1]
                    break

        user_list = [session['username'], number_of_keywords, recent_searches, genre_counts]
    ...
    if 'loggedin' in session:
        ...
        for movie in Movies:
            if title == movie['title']:
                app.logger.error("match fround")
                cursor.execute('INSERT OR IGNORE INTO watched (username, Title, Genres) VALUES (?, ?, ?)',
                               (session['username'], title, ",".join(movie['genres']),))
                conn.commit()
                return "ok"
        app.logger.error("no match found")
    return redirect(url_for('login'))
